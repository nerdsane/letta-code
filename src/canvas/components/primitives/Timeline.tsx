...
export function Timeline({
  ...
  if (events.length === 0) {
    return <div className="dsf-timeline-empty">No events to display</div>;
  }

  if (orientation === "horizontal") {
    return (
      <div
        className={`dsf-timeline dsf-timeline-horizontal dsf-timeline-${variant}`}
      >
  ...
  return (
    <div
      className={`dsf-timeline dsf-timeline-vertical dsf-timeline-${variant}`}
    >
      {events.map((event, index) => (
        <div
          key={event.id || index}
